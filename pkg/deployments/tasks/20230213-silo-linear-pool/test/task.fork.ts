...
describeForkTest('SiloLinearPoolFactory', 'mainnet', 15989794, function () {
  ...
  before('run task', async () => {
    task = new Task('20230213-silo-linear-pool', TaskMode.TEST, getForkedNetwork(hre));
  ...
  describe('create and check getters', () => {
    ...
    it('check factory version', async () => {
      const expectedFactoryVersion = {
        ...
        version: 1,
        deployment: '20230213-silo-linear-pool',
      };
  ...
  describe('read-only reentrancy protection', () => {
    ...
    before('deploy attacker', async () => {
      attacker = await deploy('ReadOnlyReentrancyAttackerSiloLP', { args: [vault.address] });
    });
