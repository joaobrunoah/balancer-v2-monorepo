...
describeForkTest('TetuLinearPoolFactory', 'polygon', 39252914, function () {
  ...
  before('run task', async () => {
    task = new Task('20230213-tetu-linear-pool', TaskMode.TEST, getForkedNetwork(hre));
  ...
  before('setup contracts', async () => {
    ...
    mainToken = await task.instanceAt('IERC20', USDT);
  ...
  describe('create and check getters', () => {
    it('deploy a linear pool', async () => {
      ...
      const { assetManager } = await vault.getPoolTokenInfo(poolId, USDT); // We could query for either frxEth or mooToken
    ...
    it('check factory version', async () => {
      const expectedFactoryVersion = {
        ...
        version: 1,
        deployment: '20230213-tetu-linear-pool',
      };
  ...
  describe('rebalancer query protection', async () => {
    it('reverts with a malicious lending pool', async () => {
      ...
      await setCode(xUSDT, getArtifact('MockTetuSmartVault').deployedBytecode);
      const mockTetuSmartVault = await deployedAt('MockTetuSmartVault', xUSDT);
      const strategyAddress = await mockTetuSmartVault.strategy();
      await setCode(strategyAddress, getArtifact('MockTetuStrategy').deployedBytecode);
  ...
  describe('read-only reentrancy protection', () => {
    ...
    before('deploy attacker', async () => {
      attacker = await deploy('ReadOnlyReentrancyAttackerTetuLP', { args: [vault.address] });
    });
