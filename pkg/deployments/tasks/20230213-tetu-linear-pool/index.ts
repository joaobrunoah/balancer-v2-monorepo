...
export default async (task: Task, { force, from }: TaskRunOptions = {}): Promise<void> => {
  ...
  if (task.mode === TaskMode.LIVE) {
    ...
    // TetuLinearPools require a Tetu Token
    const mockShareTokenArgs = ['DO NOT USE - Mock Tetu Token', 'TEST', 18, input.WETH];
    const mockShareToken = await task.deployAndVerify('MockTetuSmartVault', mockShareTokenArgs, from, force);
